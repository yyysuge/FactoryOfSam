...
class Html2PDF(object):
    ...
    def get_full_pdf(self,url_list):
        ...
        for item in url_list:
            if item["title"] == None:
                g_list.append(gevent.spawn(get_one_pdf,self.__url, u"MongoDB教程",))
                #g_list.append(Thread(target=get_one_pdf,args= (self.__url, u"MongoDB教程",)))
            if item["url"] != None and item["title"] != None:
                g_list.append(gevent.spawn(get_one_pdf, item["url"], item["title"],))
                #g_list.append(Thread(target=get_one_pdf, args=(item["url"], item["title"],)))
